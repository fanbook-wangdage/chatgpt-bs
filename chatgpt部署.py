...
#线程1
def thread1(lingpai='',bot_id='',api_key=''):
    global ms
    global xz
    global sycyid,cysycs,jgczsj,dycs,hhxxlb,hhidlb,messages,qgglb
    ...
    # 设置窗口标题
    if hwnd != 0:
        kernel32.SetConsoleTitleW("ChatGPT进程")
...
def runbot():
    global entry,getbotid,getapikey
    token=entry.get()
    botid=getbotid.get()
    apikey=getapikey.get()
    #设置标签内容
    additional_label.config(text="正在运行，详细信息请查看终端")
    #设置按钮b1禁止点击
    b1.config(state=tk.DISABLED)
    #运行线程1，传参数
    t1 = threading.Thread(target=thread1,args=(token,botid,apikey))
    t1.start()

def gk():
    #打开网址
    webbrowser.open('https://faucet.openkey.cloud/')

root = tk.Tk()
root.title("ChatGPT部署工具"+str(v))
...
label = ttk.Label(frame1, text="机器人id:")
label.grid(row=1, column=0)

label = ttk.Label(frame1, text="ApiKey:")
label.grid(row=2, column=0)

b1=ttk.Button(tab1, text="运行",command=runbot)
b1.pack(padx=6, pady=6)

b2=ttk.Button(tab1, text="获取ApiKey",command=gk)
b2.pack(padx=7, pady=6)

# 添加另一个标签，仍然使用pack布局管理器
additional_label = tk.Label(tab1, text="请确保你的机器人有白名单\n填写完参数后点击运行即可启动机器人")
...
getbotid = ttk.Entry(frame1)
getbotid.grid(row=1, column=1)

getapikey = ttk.Entry(frame1)
getapikey.grid(row=2, column=1)
...
#创建一个标签
additional_label2 = tk.Label(tab3, text="王大哥 ChatGPT部署工具-"+str(v))
